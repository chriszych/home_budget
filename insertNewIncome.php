	...
	if(isset($_SESSION['logged_id'])){
		...
		$nextYear = date('Y-m-d\TH:i', strtotime('+1 year'));
				...
				if(empty($amount) || $amount < 0.01 || $amount > 99999999.99) {
					...
					$preQuery = $db->prepare("SELECT inc_cat_name FROM income_user_category WHERE id_user = :id_user AND id_inc_user_cat = :id_cat");
					$preQuery->bindValue(':id_user', $user_id, PDO::PARAM_STR);
					$preQuery->bindValue(':id_cat', $category, PDO::PARAM_STR);
					$preQuery->execute();
					...
					exit();
						...
						$query = $db->prepare('INSERT INTO income VALUES(NULL, :id_user, :datetime, :id_inc_cat, :amount, :comment)');
						$query->bindValue(':id_user', $user_id, PDO::PARAM_STR);
						$query->bindValue(':datetime', $datetime, PDO::PARAM_STR);
						$query->bindValue(':id_inc_cat', $category, PDO::PARAM_STR);
  ...
  <main id="loginPage">
      ...
      <div class="py-3 col-5 pb-5 pt-5 text-center container">
        <div class="p-5 pb-5 pt-5 border-bottom-0">
          <p class="fw-semibold mb-0 fs-2 text-start text-success">Przychód dodany pomyślnie!</p>
		...
		<div class="modal-body p-5 pt-5 pb-5">
          <div class="d-grid gap-2 pb-5 pt-5 d-md-flex justify-content-md-center">
			<a href="./addIncome.php" class="w-100 mb-2 btn btn-lg rounded-3 btn-primary my-5 mb-5" role="button">Dodaj przychód</a>
